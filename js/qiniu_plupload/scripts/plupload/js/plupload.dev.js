	...
	plupload.Uploader = function(options) {
		...
		function onUploadFile(up, file) {
			...
			function uploadNextChunk() {
				...
				{
					if (chunkSize && features.chunks && file.size > chunkSize) { // blob will be of type string if it was loaded in memory
						curChunkSize = Math.min(chunkSize, file.size - offset);
						chunkBlob = blob.slice(offset, offset + curChunkSize);
					} else {
				...
				if (chunkSize && features.chunks) {
					...
					if (up.settings.send_chunk_number) {
						...
						if (blob != undefined)
							...
							args.chunks = Math.ceil(file.size / chunkSize);
					} else { // keep support for experimental chunk format, just in case
						args.offset = offset;
						if (blob != undefined)
							args.total = blob.size;
						else
							args.total = file.size;
					}
				}
