/*global plupload */
/*global qiniu */
var imgBaseUrl= "http://ototxz4wy.bkt.clouddn.com/";
/*global plupload */
/*global qiniu */
function FileProgress(file, targetID) {
    ...
    if (!this.fileProgressWrapper.length) {
        ...
        var operateTd = $("<td   />");
        operateTd.append("<span class='qiniu_item_del'>删除</span>");
        Wrappeer.append(operateTd);
...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    if (res.url) {
        url = res.url;
        str = "<div><a href=" +imgBaseUrl+ res.url +
        ...
        url =  encodeURI(res.key);
        var link =  res.key;
        //var link = domain + res.key;
        str = "<div><a href="+imgBaseUrl + url + " target='_blank' ><strong>下载</strong></a></div>"
            //+"<div class=hash><strong>Hash:</strong>" + res.hash + "</div>"
        ;
    }
    ...
    if(!isFile) {
        ...
        var showImg = $('<img src="static/plupload/images/loading.gif"/>');

        progressNameTd.append(Wrapper);

        if (!isImg) {
            showImg.attr('src', 'static/plupload/images/default.png');
            ...
            $(img).on('load', function () {
                ...
                function initImg(url, key, height) {
                    ...
                    modalBody.find('img').attr('src', 'static/plupload/images/loading.gif');
